var ansi = require('ansi'),
	...
	options = [
		...
		{
			names: [ 'arg', 'a' ],
			help: 'Arbitrary argument to pass to the runner',
			helpArg: 'NAME=VALUE',
			type: 'arrayOfString'
		},
		{
...
exports.findIntern = function () {
	...
	try {
		internDir = path.dirname(relative.resolve('intern', process.cwd()));
	} catch (e) {
		try {
			internDir = path.dirname(relative.resolve('intern-geezer', process.cwd()));
		} catch (e) {
			return null;
		}
	}
